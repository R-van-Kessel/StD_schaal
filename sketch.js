...
function generateQuestions() {
    ...
    for (let i = 0; i < 10; i++) {
        const scaleRatio = commonScales[floor(random(commonScales.length))];
        
        // Kies vraagtype: 1 = zoek schaal, 2 = zoek werkelijkheid, 3 = zoek model
        const questionType = floor(random(3)) + 1;
        
        let text, answer;
        let realSize, modelSize;
        
        // Bepaal eenheden en groottes
        const useKm = random() > 0.5; // 50% kans op km/cm combinatie
        
        if (useKm) {
            // Werkelijkheid in km, model in dm
            // Start met een modelgrootte en bereken werkelijkheid
            modelSize = floor(random(5, 100)); // Model tussen 5 en 100 cm (minimum 5)
            realSize = (modelSize * scaleRatio) / 10000; // dm naar km
            realSize = Math.round(realSize); // Rond af
            
            // Voorkom 0 waarden
            if (realSize === 0) realSize = floor(random(10, 101));//dit is willekeurig, kunt ook andere waardes hiervoor nemen
            
            // Herbereken modelSize voor exacte match
            modelSize = (realSize * 10000) / scaleRatio;
          
            // Herbereken modelSize voor exacte match
            modelSize = Math.round((realSize * 10000) / scaleRatio);  
          
            if (questionType === 1) {
                text = `werkelijkheid:\n ${realSize} km\nmodel: ${modelSize} m\nschaal = 1:?`;
                answer = `${scaleRatio}`;
            } else if (questionType === 2) {
                text = `schaal 1:${scaleRatio}\nmodel:${modelSize} dm\nwerkelijkheid\n in km = ?`;
                answer = `${realSize}`;
            } else {
                text = `werkelijkheid:\n ${realSize} km\nschaal 1:${scaleRatio}\nmodel in dm = ?`;
                answer = `${modelSize}`;
            ...
            // Werkelijkheid in m, model in cm (of mm)
            // Start met een modelgrootte en bereken werkelijkheid
            modelSize = floor(random(2, 100)); // Model tussen 2 en 100 cm (minimum 2)
            realSize = (modelSize * scaleRatio) / 100; // cm naar m
            realSize = Math.round(realSize); // Rond af
            
            // Voorkom 0 waarden
            if (realSize === 0) realSize = floor(random(10, 101));
            
            // Herbereken modelSize voor exacte match
            modelSize = (realSize * 100) / scaleRatio;
          
          // Herbereken modelSize voor exacte match
            modelSize = Math.round((realSize * 100) / scaleRatio);
            
            // Soms naar mm converteren voor hele kleine modellen
            if (modelSize < 10 && random() > 0.3) {  // 70% kans voor modellen onder 10cm 
                modelSize = Math.round(modelSize * 10); // naar mm
                
                // Voorkom 0 waarden
                if (modelSize === 0) modelSize = 1;
                
                // Herbereken realSize voor exacte match
                realSize = (modelSize * scaleRatio) / 1000; // mm naar m
             
              // Herbereken realSize voor exacte match
                realSize = Math.round((modelSize * scaleRatio) / 1000);  
                ...
                modelSize = Math.round(modelSize); // Rond af naar hele cm
                
                // Voorkom 0 waarden
                if (realSize === 0) realSize = floor(random(10, 101));
                
                // Herbereken realSize voor exacte match
                realSize = (modelSize * scaleRatio) / 100;
              
              // Herbereken realSize voor exacte match
                realSize = Math.round((modelSize * scaleRatio) / 100);
...
function drawBlock(block) {
    ...
    // Tekstgrootte
    if (block.isQuestion) {
        textSize(12);  // Vaste grootte voor vragen
    } else {
    ...
    if (block.text.includes('\n')) {
        ...
        let lineHeight = 16;  // Ruimte tussen regels
